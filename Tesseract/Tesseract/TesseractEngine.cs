...
namespace Tesseract
{
    ...
    public class TesseractEngine : DisposableBase
    {
        ...
        internal bool _canDispose = true;
        public bool CanDispose => _canDispose;

        public bool Stopped { get; set; }

        private HandleRef handle;

        private short processCount = 0;
        ...
        public Page Process(Pix image, string inputName, Rect region, PageSegMode? pageSegMode = null)
        {
            ...
            if (image == null) { _canDispose = true; throw new ArgumentNullException("image"); }
            if (region.X1 < 0 || region.Y1 < 0 || region.X2 > image.Width || region.Y2 > image.Height)
            { _canDispose = true; throw new ArgumentException("The image region to be processed must be within the image bounds.", "region"); }
            if (processCount > 0) { _canDispose = true; throw new InvalidOperationException("Only one image can be processed at once. Please make sure you dispose of the page once your finished with it."); }
