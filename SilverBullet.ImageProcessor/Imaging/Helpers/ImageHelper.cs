...
namespace SilverBullet.ImageProcessor.Imaging.Helpers
{
    ...
    {
        internal static Bitmap Conv3x3(Bitmap b, ConvMatrix m)
        {
        ...
        {
            using (Mat src = Mat.FromImageData(bitmap.ToByteArray(), imreadModes))
            {
                using (Mat applied = func?.Invoke(src))
                {
                    bitmap = applied.ToBitmap().Clone() as Bitmap;
                }
            }
            return bitmap;
        }
